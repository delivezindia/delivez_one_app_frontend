...
export default function AdminReturnPickupView({ onViewOrderDetail }) {
  ...
  const handleStatus = async (id, st) => {
    try {
      ...
      await updateAdminReturnStatus(id, st, { timestamp: nowIso })
      try {
        ...
        const updatedHistory = [
          { status: st, timestamp: nowIso, actor: 'Reverse Logistics Dispatcher', note: `Return pickup status marked as ${st}` },
                ...
                <tr key={b.id}>
                  <td>
                    <strong
                      ...
                      onClick={() => (onViewOrderDetail ? onViewOrderDetail(b, 'return-pickup') : setSelected(b))}
